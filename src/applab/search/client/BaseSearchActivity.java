...
public abstract class BaseSearchActivity extends ApplabActivity {
     ...
     * 
     * For Colombia, there are 3 or more digits
     * 
     * @param text
     *            the matcher
...
/**        Pattern pattern1 = Pattern.compile("[a-zA-Z]{2}[0-9]{4,5}+");
    ...
    /**
     * Method to retrieve farmer Details from the local db when updating a new farmer.
     * 
     * @return farmer details
     */
    String[] getFarmerDetails(String farmerId) {
        this.searchDatabase.open();
        String[] farmerDetails = searchDatabase.findFarmerDetailsFromFarmerLocalCacheTable(farmerId);
        this.searchDatabase.close();
