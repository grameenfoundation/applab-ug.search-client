...
public class Storage {
	...
	public Cursor selectMenuOptions(String table, String optionColumn,
			String condition) {
		// database.rawQuery("SELECT DISTINCT(" + optionColumn + ") FROM " +
		// table + " WHERE ", selectionArgs)
		return database.query(true, table, new String[] { optionColumn },
				condition, null, null, null, " MAX("
						+ MENU_ITEM_POSITION_COLUMN + ") DESC, " + optionColumn
						+ " ASC", null);
	}
	...
	public String selectContent(String menuItemId) {
		Cursor cursor = null;
		try {
			cursor = database.query(true, GlobalConstants.MENU_ITEM_TABLE_NAME,
					new String[] { Storage.MENU_ITEM_CONTENT_COLUMN },
	...
	public boolean insertContentInBatch(String table, ContentValues values) {
		...
		// Note: remember to call storage.close() - it will end any pending
		// transactions, in case there are <
	 ...
	 */
	public boolean tableExistsAndIsValid(String table, String idColumn,
			String labelColumn) {
		Cursor cursor = database.query(table, new String[] { idColumn,
				labelColumn }, null, null, null, null, null, "1");
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		private String getMenuItemTableInitializationSql() {
			...
			sqlCommand.append(" (" + Storage.MENU_ITEM_ROWID_COLUMN
					+ " CHAR(16) PRIMARY KEY, "
					+ Storage.MENU_ITEM_LABEL_COLUMN + " TEXT NOT NULL, "
					+ Storage.MENU_ITEM_MENUID_COLUMN + " CHAR(16), "
					+ Storage.MENU_ITEM_PARENTID_COLUMN + " CHAR(16), "
					+ Storage.MENU_ITEM_POSITION_COLUMN + " INTEGER, "
					+ Storage.MENU_ITEM_CONTENT_COLUMN + " TEXT, "
		...
		private String getFarmerLocalCacheTableInitializationSql() {
			...
			sqlCommand.append(" (" + Storage.FARMER_LOCAL_CACHE_ROWID_COLUMN
					+ " CHAR(16) PRIMARY KEY, "
					+ Storage.FARMER_LOCAL_CACHE_FARMER_ID + " CHAR(16), "
	...
	public Cursor getTopLevelMenuItems(String menuId) {
		Cursor itemCursor = database.query(false,
				GlobalConstants.MENU_ITEM_TABLE_NAME, new String[] {
						Storage.MENU_ITEM_ROWID_COLUMN,
						Storage.MENU_ITEM_LABEL_COLUMN,
						Storage.MENU_ITEM_ATTACHMENTID_COLUMN },
				Storage.MENU_ITEM_MENUID_COLUMN + " = ? AND ("
						+ Storage.MENU_ITEM_PARENTID_COLUMN + " IS NULL OR "
						+ Storage.MENU_ITEM_PARENTID_COLUMN + " = '' )",
				new String[] { menuId }, null, null,
				Storage.MENU_ITEM_POSITION_COLUMN + " ASC, "
						+ Storage.MENU_ITEM_LABEL_COLUMN + " ASC", null);
		return itemCursor;
	}
	...
	public Cursor getMenuList() {
		try {
			Cursor cursor = database.query(false,
					GlobalConstants.MENU_TABLE_NAME, new String[] {
							Storage.MENU_ROWID_COLUMN,
							Storage.MENU_LABEL_COLUMN }, null, null, null,
	...
	int getMenuCount() {
		Cursor cursor = database.rawQuery("SELECT COUNT(*) as total FROM "
				+ GlobalConstants.MENU_TABLE_NAME, null);
		int count = 0;
		if (cursor.moveToFirst()) {
	...
	public Cursor getChildMenuItems(String parentMenuItemId) {
		Cursor itemCursor = database.query(false,
				GlobalConstants.MENU_ITEM_TABLE_NAME, new String[] {
						...
						Storage.MENU_ITEM_ATTACHMENTID_COLUMN },
				Storage.MENU_ITEM_PARENTID_COLUMN + " = ?",
				new String[] { parentMenuItemId }, null, null,
				Storage.MENU_ITEM_POSITION_COLUMN + " ASC, "
						+ Storage.MENU_ITEM_LABEL_COLUMN + " ASC", null);
		return itemCursor;
	}
	...
	public String getFirstMenuId() {
		Cursor cursor = null;
		try {
			cursor = database.query(false, GlobalConstants.MENU_TABLE_NAME,
					new String[] { Storage.MENU_ROWID_COLUMN }, null, null,
					null, null, null, null);
			if (cursor.moveToFirst()) {
	...
	/**
	 * Delete all entries for this id and also where the parent id is this id
	 * (delete children too)
	 * 
	 * @param table
	 * @param id
	...
	public boolean deleteMenuItemEntry(String itemId) {
		return database.delete(GlobalConstants.MENU_ITEM_TABLE_NAME,
				Storage.MENU_ITEM_ROWID_COLUMN + "='" + itemId + "'", null) > 0;
	}

	/**
	 * Delete all entries in menu table for this id and also where the parent id
	 * this id (delete children too)
	 * 
	 * @param table
	 * @param id
	 * @return
	...
	public boolean deleteMenuEntry(String menuId) {
		return database.delete(GlobalConstants.MENU_TABLE_NAME,
				Storage.MENU_ROWID_COLUMN + "= '" + menuId + "'", null) > 0;
	...
	public boolean deleteUsedFarmerIds() {
		return database.delete(GlobalConstants.AVAILABLE_FARMER_ID_TABLE_NAME,
				Storage.AVAILABLE_FARMER_ID_STATUS + "=" + 1, null) > 0;
	}
	 ...
	 * @return the number of rows affected by the call to update
	 */
	public void toggleFarmerIdStatus(String farmerId, String newStatus) {
		...
		database.update(GlobalConstants.AVAILABLE_FARMER_ID_TABLE_NAME,
				farmerIdStatus, Storage.AVAILABLE_FARMER_ID_FARMER_ID + " = ?",
				new String[] { farmerId });
	}
}
